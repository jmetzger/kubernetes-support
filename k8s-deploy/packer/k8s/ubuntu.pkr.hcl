...
source "vsphere-iso" "k8s" {
  ...
  # Cloud-init Autoinstall — user-data aus diesem Verzeichnis
  cd_content = {
  ...
  # Direkt in Content Library ablegen — kein separater Export noetig
...
build {
  sources = ["source.vsphere-iso.k8s"]

  provisioner "shell" {
    execute_command = "echo 'ubuntu' | sudo -S bash -c '{{ .Vars }} {{ .Path }}'"
    scripts = [
      "${path.root}/../scripts/01-system-base.sh",
      "${path.root}/../scripts/02-kernel-modules.sh",
      "${path.root}/../scripts/03-containerd.sh",
      "${path.root}/../scripts/04-kubernetes.sh",
